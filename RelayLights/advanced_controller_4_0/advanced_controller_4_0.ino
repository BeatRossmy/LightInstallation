 ...
 * notes on channel 3 turn on and off relays on noteOn and noteOff messages
 */
...
#include "Relay.h"
//const int n=RELAYS;
Relay channels [RELAYS] = {
  {26},{27},{28},{29},{30},{31},{32},{33},
  {42},{43}//,{44},{45},{46},{47},{48},{49},
};

#include "LightEvent.h"
LightEvent events [RELAYS];
...
void handleNoteOn(byte channel, byte pitch, byte velocity) {
  ...
  if (channel==1) {
    ...
    byte rel = pitch%RELAYS;
    byte mode = (params[1]>64)?2:1;
    unsigned long t = millis() + 1 + params[0]*3;
    events[rel] = {rel,mode,t};
  }
  else if (channel==3) {
    int rel = pitch%RELAYS;
...
void setup() {
  for (int i=0; i<RELAYS; i++) channels[i].init();
